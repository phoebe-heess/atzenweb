...
with open('src/components/ThreeDMap.tsx', 'r') as f:
    lines = f.readlines()
...
for i, line in enumerate(lines):
    if '{/* Brand New telemetry dashboard panel (Real-time Inventory Tracking System) */}' in line:
        cut_index = i
        break

if cut_index != -1:
    new_lines = lines[:cut_index - 4]
    with open('src/components/ThreeDMap.tsx', 'w') as f:
        f.writelines(new_lines)
        f.write(missing_part)
    print('Fixed ThreeDMap.tsx')
else:
